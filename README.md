...
当前已完成可运行的 V3 产品概念 Demo：背景长卷加载完成后先播放独立的卷首过场，《琵琶行》题字向左隐去，完全退场后才默认以慢速匀速展开正文。用户可随时暂停，或通过抵住左右卷缘、拖动底部卷轴和方向键接管阅卷；任何主动接管都会停止自动播放，之后只由播放按钮重新开启。卷缘移动与画布方向一致，卷轴从右侧卷首向左侧结尾展开；开场题字、44 行正文与“曲终”处于同一连续空间，并分别使用开场与收尾琵琶句衔接。22 列文字弦支持抓取、横扫、分列音阶、方向音色和旋律短句。
